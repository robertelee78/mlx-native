//! Dense f16 × f32 → f32 matmul using the optional Metal tensor API when
...
pub(super) fn dense_matmul_f16_f32_with_backend(
    encoder: &mut CommandEncoder,
    registry: &mut KernelRegistry,
    device: &MlxDevice,
    src0: &MlxBuffer,
    ...
    if broadcast > i16::MAX as u32 {
        return Err(MlxError::InvalidArgument(format!(
            "dense_matmul_f16_f32_tensor: GQA broadcast ratio ({broadcast}) exceeds i16::MAX"
        )));
    }
    ...
    let use_tensor =
        tensor_pipeline_available(backend, "hf2q_dense_mm_f16_f32_tensor", registry, device)?;
    let kernel_name = if use_tensor {
        "hf2q_dense_mm_f16_f32_tensor"
    } else {
        "hf2q_dense_mm_f16_f32_fallback"
    };
