...
pub fn dispatch_fused_gate_up_silu_q4_K(
    ...
    let weight_bytes = (args.intermediate_size as usize)
        ...
        .and_then(|value| value.checked_mul(BLOCK_Q4_K_BYTES as usize))
        .ok_or_else(|| {
            MlxError::InvalidArgument("fused_gate_up_silu_q4_K: weight size overflow".into())
        })?;
    let input_bytes = (args.hidden_size as usize)
        .checked_mul(args.m as usize)
        .and_then(|value| value.checked_mul(4))
        .ok_or_else(|| {
            MlxError::InvalidArgument("fused_gate_up_silu_q4_K: input size overflow".into())
        })?;
    let output_bytes = (args.intermediate_size as usize)
        .checked_mul(args.m as usize)
        .and_then(|value| value.checked_mul(4))
        .ok_or_else(|| {
            MlxError::InvalidArgument("fused_gate_up_silu_q4_K: output size overflow".into())
        })?;
...
#[allow(clippy::too_many_arguments, non_snake_case)]
pub fn dispatch_fused_gate_up_silu_q4_K_with_trace(
    encoder: &mut CommandEncoder,
    registry: &mut KernelRegistry,
    device: &MlxDevice,
    gate_w: &MlxBuffer,
