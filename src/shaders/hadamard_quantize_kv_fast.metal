...
// iter-27: 8-bit calibrated codebook (256 centroids, empirical Gemma-4-27B KV fit).
// PLACEHOLDER: values are patched by iter27_audit after Lloyd-Max on real distribution.
// Must match CODEBOOK_HB_8BIT_CALIBRATED in flash_attn_vec_tq_hb.metal exactly.
// ITER27_CALIBRATED_CODEBOOK_BEGIN
constant float CODEBOOK_8BIT_CALIBRATED[256] = {
    -5.0652659f, -4.6836997f, -4.4467193f, -4.2715508f,
    -4.1311907f, -4.0132856f, -3.9111092f, -3.8205780f,
    -3.7390194f, -3.6645851f, -3.5959415f, -3.5320936f,
    -3.4722785f, -3.4158977f, -3.3624729f, -3.3116156f,
...
// iter-27: calibrated boundaries (255 values, midpoints of calibrated centroids).
// PLACEHOLDER: patched by iter27_audit. Must match encode/decode convention.
constant float BOUNDARIES_8BIT_CALIBRATED[255] = {
    -4.8744828f, -4.5652095f, -4.3591350f, -4.2013707f,
    -4.0722382f, -3.9621974f, -3.8658436f, -3.7797987f,
    -3.7018022f, -3.6302633f, -3.5640175f, -3.5021860f,
    -3.4440881f, -3.3891853f, -3.3370443f, -3.2873106f,
    ...
    -1.1675909f, -1.1404016f, -1.1132168f, -1.0860362f,
    -1.0588593f, -1.0316859f, -1.0045155f, -0.9773478f,
    -0.9501822f, -0.9230185f, -0.8958564f, -0.8686959f,
    -0.8415367f, -0.8143786f, -0.7872215f, -0.7600653f,
    -0.7329100f, -0.7057553f, -0.6786014f, -0.6514481f,
    -0.6242953f, -0.5971430f, -0.5699912f, -0.5428397f,
    -0.5156887f, -0.4885378f, -0.4613872f, -0.4342368f,
    -0.4070854f, -0.3799332f, -0.3527802f, -0.3256266f,
    -0.2984724f, -0.2713176f, -0.2441623f, -0.2170068f,
    -0.1898510f, -0.1626950f, -0.1355389f, -0.1083828f,
    -0.0812268f, -0.0540710f, -0.0269153f,  0.0000000f,
     0.0269153f,  0.0540710f,  0.0812268f,  0.1083828f,
     0.1355389f,  0.1626950f,  0.1898510f,  0.2170068f,
     0.2441623f,  0.2713176f,  0.2984724f,  0.3256266f,
     0.3527802f,  0.3799332f,  0.4070854f,  0.4342368f,
     0.4613872f,  0.4885378f,  0.5156887f,  0.5428397f,
     0.5699912f,  0.5971430f,  0.6242953f,  0.6514481f,
     0.6786014f,  0.7057553f,  0.7329100f,  0.7600653f,
     0.7872215f,  0.8143786f,  0.8415367f,  0.8686959f,
     0.8958564f,  0.9230185f,  0.9501822f,  0.9773478f,
     1.0045155f,  1.0316859f,  1.0588593f,  1.0860362f,
     1.1132168f,  1.1404016f,  1.1675909f,  1.1947853f,
     1.2219851f,  1.2491911f,  1.2764037f,  1.3036237f,
     1.3308517f,  1.3580886f,  1.3853351f,  1.4125923f,
...
struct HadamardQuantizeHbParams {
    ...
    // iter-27: 1 = use CODEBOOK_8BIT_CALIBRATED; 0 = N(0,1)-fit CODEBOOK_8BIT.
    // Only meaningful at codebook_bits==8.
    ...
    uint  tiisg [[thread_index_in_simdgroup]])
{
    ...
    for (ushort i = 0; i < EPT; i++) {
        ...
        } else if (params.codebook_calibrated != 0u) {
            // 8-bit calibrated: iter-27 empirical codebook, 8-level binary search
            idx = (v > BOUNDARIES_8BIT_CALIBRATED[127]) ? 128 : 0;
            idx += (v > BOUNDARIES_8BIT_CALIBRATED[idx + 63]) ? 64 : 0;
            idx += (v > BOUNDARIES_8BIT_CALIBRATED[idx + 31]) ? 32 : 0;
            idx += (v > BOUNDARIES_8BIT_CALIBRATED[idx + 15]) ? 16 : 0;
            idx += (v > BOUNDARIES_8BIT_CALIBRATED[idx + 7])  ?  8 : 0;
            idx += (v > BOUNDARIES_8BIT_CALIBRATED[idx + 3])  ?  4 : 0;
            idx += (v > BOUNDARIES_8BIT_CALIBRATED[idx + 1])  ?  2 : 0;
            idx += (v > BOUNDARIES_8BIT_CALIBRATED[idx])      ?  1 : 0;
        } else {
            // 8-bit N(0,1)-fit: default codebook, binary search with 8 levels
