...
// loop count bumped from 16 → 32. Same algorithm, same shmem layout
// (sized via params at dispatch time — caller passes K=256).
//
// Note: chunk_o's MMA accumulators (`bo_acc[4]`, `bA_acc[8]`) are sized
// by V-tile and BT-tile respectively, NOT K-tile. The K-reduction is a
// SUM across all K-tiles, so larger K means more iterations but no
// additional accumulator registers. This makes the K=128 → K=256 port
// minimal vs the chunk_inter_state kernel (which had a K-indexed
// accumulator array doubled from 16 to 32).
kernel void gated_delta_net_chunk_o_bf16_k256(
    ...
    // ===================================================================
    {
        ...
        const uint num_k_tiles = 32u; // iter 20 K=256 (K=256/8 = 32)
