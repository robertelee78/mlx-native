...
use mlx_native::ops::deepseek_sparse_prefill_mask::{
    self, dispatch_deepseek_sparse_prefill_mask_f16, DeepSeekSparsePrefillMaskParams,
};
use mlx_native::ops::flash_attn_prefill::FlashAttnPrefillParams;
use mlx_native::ops::flash_attn_prefill_blk::{
    self, dispatch_flash_attn_prefill_blk_f16, BlkParams,
};
use mlx_native::ops::flash_attn_prefill_d512::{
    self, dispatch_flash_attn_prefill_f16_d512_with_blk_and_sinks,
...
fn f16_d512_skip_map_is_byte_identical_with_sinks() {
    ...
    let mask = device
        .alloc_buffer(rows * kv_len * 2, DType::F16, vec![rows, kv_len])
        .unwrap();
    ...
    for (index, (plain, skipped)) in plain_values.iter().zip(skipped_values).enumerate() {
        assert_eq!(
            plain.to_bits(),
            skipped.to_bits(),
            "D512 skip output drift at element {index}"
        );
    }
}
