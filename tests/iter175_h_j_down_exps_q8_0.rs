...
//!
//! This cross-project benchmark is opt-in because it compiles the external
//! `/opt/llama.cpp` Metal source. Run it against a known-compatible checkout:
//! `cargo test --release --test iter175_h_j_down_exps_q8_0 -- --ignored --nocapture`
...
fn build_metallib(src: &str, includes: &[&str], suffix: &str, out_dir: &str) -> PathBuf {
    ...
    let mut args: Vec<String> = vec!["-sdk".into(), "macosx".into(), "metal".into(), "-O3".into()];
    for inc in includes {
        args.push("-I".into());
...
fn bench(
    ...
    for _ in 0..WARMUP {
        ...
        for (s, b) in buffers {
            enc.set_buffer(*s, Some(b.metal_buffer()), b.byte_offset());
        }
        enc.set_bytes(
            params_slot,
...
fn h_j_down_exps_q8_0() {
    ...
    let peer_lib = build_metallib(
        ...
        &[
            "/opt/llama.cpp/ggml/src/ggml-metal",
            "/opt/llama.cpp/ggml/src",
            "/opt/llama.cpp/ggml/include",
        ],
        "peer_q8_0",
        &out_dir,
    );
    ...
    let hf2q_tg = MTLSize::new(
        (N as u64 + hf2q_align - 1) / hf2q_align,
    ...
    eprintln!(
        "\n[H-J] gemma4 ffn_down_exps shape: N={} K={} top_k={} (Q8_0)",
        N, K, TOP_K
    );
    eprintln!(
        "[H-J] hf2q: tg={:?}, threads={:?}",
        (hf2q_tg.width, hf2q_tg.height, hf2q_tg.depth),
        (hf2q_threads.width, hf2q_threads.height, hf2q_threads.depth)
    );
    eprintln!(
        "[H-J] peer: tg={:?}, threads={:?}",
        (peer_tg.width, peer_tg.height, peer_tg.depth),
        (peer_threads.width, peer_threads.height, peer_threads.depth)
    );
    ...
    let v = if delta.abs() < 2.0 {
        "TIED (±2%)"
    } else if delta < 0.0 {
        "PEER FASTER — gemma4 down_exps lever exists"
    } else {
        "HF2Q FASTER — kernel quality not the bottleneck here"
    };
    eprintln!("  verdict: {}", v);
}
